...
for student_model in cfg["student_models"]:
    print(f"\n{'='*60}")
    print(f"Training student: {student_model}")
    print(f"{'='*60}\n")

    # Create results directory with hyperparameters
    student_name = student_model.split("/")[-1]
    ...
    training_config = {
        ...
        "_student_name": student_model,  # for eval callback
    }
    ...
    student_model_obj = AutoModelForCausalLM.from_pretrained(student_model_name, dtype = precision)

    student_tokenizer = AutoTokenizer.from_pretrained(student_model_name)
    if student_tokenizer.pad_token_id is None:
      student_tokenizer.pad_token_id = student_tokenizer.eos_token_id
    student_model_obj.config.pad_token_id = student_tokenizer.pad_token_id
